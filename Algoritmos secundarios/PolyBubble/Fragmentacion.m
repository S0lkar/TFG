...
function [C_S, I_S, C_EC, I_EC, C_PI, I_PI, C_Sub, I_Sub, Len] = Fragmentacion(C_S, I_S, C_EC, I_EC, I_pt, C_pt)
    ...
    if es_anterior
        ...
        ind = find(I_EC(mod(pos_primero, length(I_EC))+1) == I, 1);
    end
    ...
    es_anterior = find(I_EC(mod(pos_ultimo-2, length(I_EC))+1) == I, 1);
    if es_anterior
        ind_sig = es_anterior;
    else % es el posterior.
        ind_sig = find(I_EC(mod(pos_ultimo, length(I_EC))+1) == I, 1);
    end
